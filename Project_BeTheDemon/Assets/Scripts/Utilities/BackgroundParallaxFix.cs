...
public class BackgroundParallaxFix : MonoBehaviour
{
	...
	void Start()
	{
		...
		for (int i = 0, idx = 0; i < max; i++, idx++)
        {
			...
			parallaxFactor[i] = backgroundCnt - 1 - idx;//시차 요소 설정
	...
	void Update()
	{
		...
		if (pd.inGameState != InGameState.Start &&
			...
			pd.heroPosX >= cam.ViewportToWorldPoint(new Vector3(0.5f, 0.0f, 0.0f)).x)//화면상 영웅 위치
			pd.cameraSpeed = ud.heroSpeed + 0.4f;
		else
			pd.cameraSpeed = 0.0f;
	}
	...
	void BackGroundScrolling(float direction, int index)
	{
		...
		else if (direction > 0.0f)  // ->	// current direction
		{
	...
	public void InsertCamObject(Camera cam)
    {
		this.cam = cam;
